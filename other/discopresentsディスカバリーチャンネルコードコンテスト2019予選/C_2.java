		...
		public void solve(int testNumber, InputReader in, PrintWriter out) {
			...
			for (int i = 0; i < m-1; i++) {
				if (list.get(i).first != list.get(i).second) {
					...
					ans %= MOD;
					diff += Math.min(list.get(i).first, list.get(i).second);
				} else {
					long A = list.get(i+1).first-list.get(i).first;
					long B = list.get(i).second-list.get(i+1).second;
					ans += power(A, 10, MOD) * power(B, 10, MOD);
					ans %= MOD;
					diff += Math.min(list.get(i).first, list.get(i).second);
				}
			}

			if (list.get(m-1).first != list.get(m-1).second) {
				long A = list.get(m-1).first;
				long B = list.get(m-1).second;
				ans += 2 * power(A, 10, MOD) * power(B, 10, MOD);
				ans %= MOD;
				diff += Math.min(list.get(m-1).first, list.get(m-1).second);
			} else {
				long A = list.get(m-1).first;
				long B = list.get(m-1).second;
				ans += power(A, 10, MOD) * power(B, 10, MOD);
