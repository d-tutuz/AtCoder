...
public class C {
	...
	static class TaskX {
		...
		public void solve(int testNumber, MyInput in, PrintWriter out) {
			...
			for (int k = 0; k < n; k++) {
				char[] a = A[k].toCharArray();
				char[] b = B[k].toCharArray();
			...
			out.println(sb.toString().length() != 26 ? "-1" : sb.toString());
		...
		void debug(Set<Character>[] g, PrintWriter out) {
			for (int idx = 0; idx <= 25; idx++) {
				for (Character c : g[idx]) {
					out.printf("%c -> %c\n", (char)(idx + 'a'), c);
				}
			}
		}
