...
public class C_2 {
	...
	static class TaskX {
		...
		public void solve(int testNumber, MyInput in, PrintWriter out) {
			...
			DisjointSet set = new DisjointSet(1000000);
			for (int i = 0; i < h; i++) {
				for (int j = 0; j < w; j++) {
					for (int k = 0; k < 4; k++) {
						int mh = i + mh4[k];
			...
			for (int i = 0; i < list.size(); i++) {
				boolean done = false;
				for (int j = 0; j < i; j++) {
					if (set.same(f(list.get(i).h, list.get(i).w), f(list.get(j).h, list.get(i).w))) done = true;
				}
				if (!done) {
					memo[f(list.get(i).h, list.get(i).w)] = dfs(list.get(i).h, list.get(i).w, new boolean[h][w], B)/2;
					ans += memo[f(list.get(i).h, list.get(i).w)];
				} else {
					ans += memo[set.root(f(list.get(i).h, list.get(i).w))];
				}
			}
