...
public class B_2 {
	...
	static long check(long v, long[] a) {
	...
	public static int[] prefSum(int[] la) {
		...
		for (int i = 0; i < la.length; i++)
			result[i] = (i == 0) ? la[i] : result[i - 1] + la[i];
		return result;
	}
	...
	public static long[] prefSum(long[] la, long mod) {
		...
		for (int i = 0; i < la.length; i++)
			result[i] = (i == 0) ? (la[i] + mod) % mod
					: (result[i - 1] + la[i] + mod) % mod;
		return result;
	}

	public static int lastLowerIndex(long[] array, long lo) {
		return lastLowerOrEqualIndex(array, lo - 1);
	}
	...
	private static void mergesortIndex(Integer[] index, long[] a, int s, int e) {// uwi's
		...
		int i = s, j = h;
		for (; i < h && j < e;)
			stem[p++] = a[(int) index[i]] < a[(int) index[j]] ? index[i++]
					: index[j++];
		while (i < h)
			stem[p++] = index[i++];
		while (j < e)
