	...
	static class TaskX {
		...
		public void solve(int testNumber, MyInput in, PrintWriter out) {
			...
			while (!pq.isEmpty()) {
				...
				if (p.cost > cost[p.n][p.time]) continue;

				for (P pp : g[p.n]) {
					if (cost[p.n][p.time] + pp.c < cost[pp.t][(p.time + pp.c) % 28]) {
						cost[pp.t][(p.time + pp.c) % 28] = cost[p.n][p.time] + pp.c;
						pq.add(new T(pp.t, (p.time + pp.c) % 28, cost[pp.t][(p.time + pp.c) % 28]));
					}
				}
			}
