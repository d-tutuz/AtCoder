...
public class C {
	...
	static class TaskX {
		...
		public void solve(int testNumber, MyInput in, PrintWriter out) {
			...
			long cur = H, ans = 0;
			for (int i = 0; i < n-1; i++) {
				long d = in.nextLong(), heal = in.nextLong();
				pq.add(new P(heal, i));
				...
				while (cur <= 0) {
					...
					long count = Math.min(-cur / p.heal + 1, (H - max) / p.heal);
					ans += count;
					cur += count * p.heal;

					seg.add(count * p.heal, p.idx, i+1, 0, 0, seg.size());
					pq.add(p);
				}
			}
		...
		class P implements Comparable<P> {
			...
			public int compareTo(P o) {
				return -Long.compare(this.heal, o.heal);
			}
			@Override
			public String toString() {
				return "P [h=" + heal + ", idx=" + idx + "]";
			}
		}
